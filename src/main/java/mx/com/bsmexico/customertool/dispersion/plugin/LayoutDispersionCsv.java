...
import mx.com.bsmexico.customertool.dispersion.plugin.LayoutDispersionCsv.DescriptionLayout;


public class LayoutDispersionCsv extends TableView<DescriptionLayout> implements ImportTarget<DescriptionLayout> {

	private final ObservableList<DescriptionLayout> data = FXCollections.observableArrayList();

	public LayoutDispersionCsv() {
		super();
		setPrefSize(800, 600);
		configColumns();
		this.setEditable(true);
		final ClassLoader classLoader = getClass().getClassLoader();
		final InputStream input = getClass().getResourceAsStream("/instrucciones/instrucciones-layout-dispersion-csv.csv");
		File file = new File("cfg/instrucciones-layout-dispersion-csv.csv");
	    try {
	    ...
	    String fileName = getJarDir()+"/../cfg/instrucciones-layout-dispersion-csv.csv";
		...
		final CSVImporter<DescriptionLayout> importCsv = new CSVImporter<DescriptionLayout>(this) {
			...
			protected DescriptionLayout getInstance(List<String> records) {
				...
				desc.setObservaciones(records.get(2));
				desc.setObligatorio(records.get(3));
				return desc;
			}
	...
	private void configColumns() {
		...
		this.getColumns().addAll(getSimpleColumn("Campo", "campo", 150),
				getSimpleColumn("Características", "caracteristicas", 170),
				getSimpleColumn("Observaciones", "observaciones", 338), getSimpleColumn("Obligatorio", "obligatorio", 150));
	}
